...
bodyLenThresh = 1        # bodyLenThresh:This is the fraction of total body length a fly
...
feedFrame     = 60         # feedFrame is the window size in secs for grabbing the mean position of a dye-band (reduces noise) 
...
def printBanner():
    ...
    # be processed.
    print("   Noah.py - A script to analyzes data from the ARC\n"
    ...
    print("            -------------------\n"
          "                 Noah 15.0\n"
          "            -------------------\n"
          "   A fly behavioral data processor by Keith Murphy \n"
          "                                                   \n"
...
def commandMenu():
    ...
    while True:
        ...
        print("\t 4.  Bin Activity Data")
        print("\t 5.  Bin Feeding Data")
        print("\t 6.  Bin Sleep Data")
        print("\t 7.  Generate Heat Map Matrix")
        print("\t 8.  Feeding Proximal Sleep")
        print("\t 9.  Target Zone Count")
        print("\t 10. Generate Kymograph")
        print("\t 11. Collective Behavior")
        print("\t 12. Export XY coordinates")
        print("\t 13. Use Virtual beam")
        print("\t 14. Arousal threshold")
        ...
        try:
            ...
            if option   == 0:
                ...
                featureChoice = input("Sleep, activity, or inactivity surrounding feeding events (s/a):")
                if featureChoice == "a":
                    feature = activity
                ...
                elif featureChoice == "g":
                    feature = groom
                    featureTitle = "grooming"
                elif featureChoice == "y":
                    feature = Y
                    featureTitle = "yCoord"
                elif featureChoice == "ge":
                    feature = groomErr
                    featureTitle = "groomErr"
                elif featureChoice == "vis":
                    feature = visData
                    featureTitle = "visData"
                else:
                    feature = sleep
                    featureTitle = "sleep"
                    print("Default: sleep")
...
def getInput():
    ...
    try:
        ...
        errorMsg = "Error: Unable to open file: '" + inputFile + "'. Try \n"
        "removing spaces or hyphens in file name." 
...
def feedLocal(mealValues,mealDurations,mealTimes,feature,featureTitle,readsPerSec,shortFileName):
    ...
    if sort == "y":
        ...
        toExcelNamed(finalList,featureTitle+str(timePush)+"push_"+str(step)+"step"+"_"+shortFileName)
        absList = transpose(absList)
        toExcelNamed(absList,shortFileName + "_abs")
...
def synth(sleep,activity,mealTimes,mealValues,mealDurations,readsPerSec,workList,useFileName):
    ...
    # Many of these measurements will not show up in the output unless indicated in statsList at the end of this function
        ...
        toExcelNamed(statsList,"stats_" + useFileName )
...
##        actBins = normalizeToMean(actBins)
    ...
    for i in range(len(actBins)):
        actBins[i].insert(0,header[i])

    actBins.insert(0,title)
    finalAct= []
    addWord = ["Total Activity Bins (seconds)",""]
    finalAct.append(addWord)
    for i in range(len(actBins)):
        finalAct.append(actBins[i])

    toExcelNamed(finalAct,"actBins_" + str(int(binSize/60))+"_mins" + shortFileName)
...
##    norm = input("Normalize Bins to Mean?(y/n): ")
##    if norm == "y" or norm == "yes" or norm == "YES" or norm == "Yes":
##        sleepBins = normalizeToMean(sleepBins)


    sleepBins = transpose(sleepBins)
    for i in range(len(sleepBins)):
        sleepBins[i].insert(0,header[i])

    sleepBins.insert(0,title)

    finalSleepDepth = []
    addWord = ["Total Sleep Bins (seconds)",""]
    finalSleepDepth.append(addWord)
    for i in range(len(sleepBins)):
        finalSleepDepth.append(sleepBins[i])

    print("NOTE: Data is expressed as seconds spent in sleep state during period")
    toExcelNamed(finalSleepDepth,"sleepBins_" + str(int(binSize/60))+"mins_"+shortFileName)
...
def mealRelation(mealTimes,mealValues,mealDurations,feedFileName,activity,sleep,readsPerSec):
        ...
        for meal in range(len(mealTimes[fly])):
            ...
            if meal > 0:
                ...
                try:
                    ...
                    sleepSinceLast.append(sum(sleep[fly][last:current]))
                except:
                    actSinceLast.append("")
            ...
            try:
                if meal < len(mealTimes[fly]):
                    ...
                    sleepToNext.append(sum(sleep[fly][current:nex]))
    ...
    final = [flyNum,mealSizes,mealTime,mealDurs,timeSinceLast,timeToNext,actSinceLast,sleepSinceLast,actToNext,sleepToNext]

...
feedFrame	 = 10		  # feedFrame is the window size in secs for grabbing the mean position of a dye-band (reduces noise) 
...
MEALPULL_SD	  = 5	   # The meal pull sd for recalculating noise sd's
...
def printBanner():
	...
	print("			-------------------\n"
		  ...
		  " is "+str(bodyLenThresh*100)+"% and can be changed within the script under the constants section \n\n\n"
...
def commandMenu():
	...
	while True:
		...
		print("\t 14. Sleep Frequency / Length")
		print("\t 15. Groom Data")
		print("\t 16. Meal Relation")
		...
		try:
			...
			if option   == 0:
				...
				file																						  = getInput()
				activity,angularVelocity,readsPerSec,workList,shortFileName,X,Y,zeitStart,stim,feedData,pixPerCM  = parseData(file)
				...
				synth(sleep,activity,mealTimes,mealValues,mealDurations,readsPerSec,workList,useFileName)
				...
				try:
					freqArray, lengthArray = sleepFreqLengthArray(sleep)
					feature = freqArray
					featureTitle = "sleep_frequency"
					feedLocal(mealValues,mealDurations,mealTimes,feature,featureTitle,readsPerSec,shortFileName)
					feature = lengthArray
					featureTitle = "sleep_length"
					feedLocal(mealValues,mealDurations,mealTimes,feature,featureTitle,readsPerSec,shortFileName)
				except:
					print('Error: check sleep frequency/length array function')
				...
				mealRelation(mealTimes,mealValues,mealDurations,feedFileName,activity,sleep,readsPerSec)
...
def changeConstants():
	...
	while True:
		c = int(input ("\n Which constant would you like to change? \n\n"
			...
			"\t 2. body length threshold for movement\t\t"+str(bodyLenThresh)+"\n"
			"\t 3. no. of frames to average for feeding\t"+str(feedFrame)+"\n"
			"\t 4. centimeters per microliter\t\t\t"+str(CM_PER_UL)+"\n"
			"\t 5. meal pull standard deviation\t\t"+str(MEALPULL_SD)+"\n"
			"\t 6. final standard deviation\t\t\t"+str(FINAL_SD)+"\n\n"
			"\t 0. return to main menu\n\n"))
		if c == 1:
			pixPerCM = input("Enter new pixPerCM: ")
		elif c == 2:
			bodyLenThresh = input("Enter new body length threshold: ")
		elif c == 3:
			feedFrame = input("Enter new feedFrame: ")
		elif c == 4:
			CM_PER_UL = input("Enter new CM_PER_UL: ")
		elif c == 5:
			MEALPULL_SD = input("Enter new MEALPULL_SD: ")
		elif c == 6:
			FINAL_SD = input("Enter new FINAL_SD: ")
		elif c == 0:
			break
		else:
			print ("input error.")
...
def getInput():
	...
	try:
		...
		print("pixels per CM set to "+str(pixPerCM))
...
def parseData(file):
	...
	X,Y = getCoordinates(animalTrack)	# gets x,y coordinates and optionally applies a sorting procedure for identity switching
	...
	for fly in range(len(X)):
		...
		for read in range(len(X[fly])):
			...
			y2 = Y[fly][read]
			try:
				singleDistance = distance(x1, y1, x2, y2)			 # Distance function found above calculates simple Euclidean Distance
			except:
				singleDistance = 0
...
def parseMealData(file):
	...
	try:
		...
		if False == isLabeled:
			...
			itemsPerFly = mealList[0].count("")
			itemCount   = len(mealList[0])
			itemsPerFly = int(round(itemCount / (itemCount - itemsPerFly),0))
			mealList.pop(0)
		...
		for reading in range(len(activity[fly])):
			if activity[fly][reading] == 0:
				try:
					...
					sleepIndiv.append(0)  # This exception is run when there are no elements in sleepIndiv[-1] or first frame
...
def feedLocal(mealValues,mealDurations,mealTimes,feature,featureTitle,readsPerSec,shortFileName):
	...
	if sort == "y":
		...
		finalList = [["fly#","meal duration(s)", "meal size(ul)", "meal time", "time since last meal(h)", "P(sleep) pre", "P(sleep) post"]]+finalList
		toExcelNamed(finalList,"prandial"+featureTitle+"_"+str(timePush)+"sWindow_"+str(step)+"sBin_"+shortFileName)
		absList = transpose(absList)
		toExcelNamed(absList,"prandial"+featureTitle+"_"+str(timePush)+"sWindow_"+str(step)+"sBin_"+shortFileName+"_abs")
...
def arousal(stim,activity,inactivity,readsPerSec,shortFileName,mealTimes=0,mealValues=0):
	...
	iDur		  = inactivity
	try:
		inactMin	  = int(input("Enter minimum time(seconds) a fly can be inactive to be considered for stimulus response: "))
	except:
		print('Default: minimum time set to 1 second')
		inactMin = 1
	try:
		inactMax	  = int(input("Enter maximum time(seconds) a fly can be inactive to be considered for stimulus response: "))
	except:
	...
	timeToRespond = int(input("Enter seconds within which an animal must move to be considered aroused by a given stimulus: "))
...
def sleepFreqLengthArray(sleep):
	...
	for fly in range(len(sleep)):
		...
		boutFreqs.append(indivFreq)
		boutLengths.append(indivLength)

	return(boutFreqs,boutLengths)
...
def synth(sleep,activity,mealTimes,mealValues,mealDurations,readsPerSec,workList,useFileName):
	...
	RI24		   = int(RI12*2)
	short,med,long = sleepDepth(sleep,readsPerSec)
	...
	# Many of these measurements will not show up in the output unless indicated in statsList at the end of this function
		...
		for i in range(flies):
			...
			try:
				for n in range(len(timeIndex)):
					if (timeIndex[n] % 86400) <= 43200:
						lightsOnDeepSleep.append(long[i][n])
					else:
						lightsOffDeepSleep.append(long[i][n])
		...
		statsList = [flyCount,Activity,Sleep,Feed,numMeal,avgMeal,avgDuration,sleepBouts,avgSleepLength,sleepLatency,mealOnsetSleep,mealSleep,
					dTotFeed,nTotFeed,dAvgMeal,nAvgMeal,dFreq,nFreq,daySleep,nightSleep,daySleepLength,nightSleepLength,daySleepFreq,
					nightSleepFreq,dayActivity,nightActivity,iMealInts,efficiencyIndex,strideDistances,strideLengths,velocity,strideCount,firstMeal,
					secondMeal,thirdMeal,fourthMeal,maxMeal,fastingPer,firstMealTime,secondMealTime,thirdMealTime,firstSleep]
...
def feedBins(mealTimes,mealValues,mealDurations,feedFileName):
	...
	try:
		...
		# if addFreq == "y":
		# 	toExcelNamed[masterMealFreq,"freqBins_" + str(int(binSize*60))+"mins_"+feedFileName)
		# if addSize == "y":
...
def collective(X,Y,activity,readsPerSec,shortFileName):
	...
	startleInact  = int(1 * readsPerSec)			# min duration which flyB must be inactive before being touched by flyA to be considered for startle
			...
			if flyA != flyB:   # stops program from checking behaviors against itself
				...
				for read in range(len(activity[flyB])):
					...
					if minSep < dis < maxSep:
						if activity[flyA][read]*readsPerSec > minVelocity and activity[flyB][read]*readsPerSec > minVelocity:
					...
					try:
						if touchingVector[-1] == 1 and touchingVector[-2] == 0:  # checks if this was the moment of touch initiation rather than some period in the middle of an extended touching
							touchVector.append(1)
							if sum(activity[flyB][read-startleInact:read]) == 0 and sum(activity[flyB][read:read + startleDelay]) > 0:
							...
							if sum(activity[flyA][read-startleInact:read]) == 0 and sum(activity[flyA][read:read + startleDelay]) > 0:
				...
				chaseVector = filterFragments(chaseVector,minDuration) # This takes only chase events which last greater than min chase duration
	...
	finalOut = [flyNum,totalCluster,totalTouch,totalTouching,totalChase,totalStartle,totalStartled,totalWakeup,totalWoken,totalSleep,totalActivity]
		...
		try:
			if splitQ == "yes" or splitQ == "y":
				...
				for i in range(round(len(yCo[0])/split)):			# rounds time series count to nearest whole time piece
					title = [str("Time Group " + str(i)),""]		 # for instance: a 23 hr experiment split into 12 hr sections will give 2 12hr time pieces instead of 1
					Quads.append(title)
...
def hotZone(workList,readsPerSec,numEntries,shortFileName):
	...
	try:
		...
		for fly in range(len(xCo)):
			flyAttempts = []
			for read in range(len(xCo[fly])):
				if xMins[fly] < xCo[fly][read] < xMaxs[fly] and yMins[fly] < yCo[fly][read] < yMaxs[fly]:	  #checks if fly is inside 'hot zone'
					try:
						if xCo[fly][read+1] < xMins[fly] or xCo[fly][read+1] > xMaxs[fly] or yCo[fly][read+1] < yMins[fly] or yCo[fly][read+1] > yMaxs[fly]:   #checks if fly left 'hot zone' the read after being inside
...
def mealRelation(mealTimes,mealValues,mealDurations,feedFileName,activity,sleep,readsPerSec):
	...
	final = [flyNum,mealSizes,mealTime,mealDurs,timeSinceLast,timeToNext,actSinceLast,actToNext,sleepSinceLast,sleepToNext]
	...
	if doDiv == "y":
		...
		for i in range(len(bulkList)):
			bulkList[i]=[["fly#","meal size (ul)","meal time", "meal duration (sec)", "time since last meal (hr)", "time to next meal (hr)", "activity since last meal (cm)", "activity until next meal (cm)", "sleep since last meal (hr)", "sleep until next meal (hr)"]]+bulkList[i]
			toExcelNamed(bulkList[i],"batch_" + str(i+1) + "_" + "mealRelation_"+feedFileName)
	else:
		toExcelNamed([["fly#","meal size (ul)","meal time", "meal duration (sec)", "time since last meal (hr)", "time to next meal (hr)", "activity since last meal (cm)", "activity until next meal (cm)", "sleep since last meal (hr)", "sleep until next meal (hr)"]]+final,"mealRelation_"+feedFileName)
...
def normalizeZeroOne(matrix):
	...
	for i in range (len(matrix)):
		matrix[i] = stats.zscore(matrix[i])
	return matrix
...
def prepareDataMatrix(mealData, ulConversion, frameOffset, intervalOffset):
	...
	for i in range(0, len(capillaryIds)):
		...
		for j in range(0, len(sortedMeals)):
			...
			mealDurationsVector.append(mealDur * intervalOffset * 3600)

...
feedFrame     = 20         # feedFrame is the window size in secs for grabbing the mean position of a dye-band (reduces noise) 
...
def getInput():
	...
	try:
		...
		shelf = input ("Middle shelf? Bottom shelf? Enter your own pixPerCM? (1/2/3) ")

		if "mid" in shelf or shelf == "1":
			pixPerCM = 31
			print ("pixPerCM set to 31")
		elif "bo" in shelf or shelf == "2":
			pixPerCM = 26
			print ("pixperCM set to 26")
		else:
			del pixPerCM
...
def mealRelation(mealTimes,mealValues,mealDurations,feedFileName,activity,sleep,readsPerSec):
    ...
    for i in range(len(bulkList)):
        bulkList[i]=[["fly#","meal size (ul)","meal time", "meal duration (sec)", "time since last meal (hr)", "time to next meal (hr)", "activity since last meal (cm)", "sleep since last meal (hr)", "activity until next meal (cm)", "sleep until next meal (hr)"]]+bulkList[i]

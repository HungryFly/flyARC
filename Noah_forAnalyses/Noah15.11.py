"""   Noah.py - A script analyze and structure Autocafe software output (created by Keith Murphy)
   ...
   Copyright (C) 2019 Keith Murphy, Scarlet Park, & James Quinn
 ...
 5/10/2019 update:
 	changed mode to median for getting rid of far outliers for cleaning food tracking
 	default no. of frames to average to 60
	default meal pull SD to 4.5

 7/25/2019 update:
 	There's now an option to calculate activity surrounding sleep events
"""
...
sleepDef = 300  # Defines a 5 min sleep bout as the default
...
MEALPULL_SD = 4.5  # The meal pull sd for recalculating noise sd's
...
def commandMenu():
    ...
    while True:
        ...
        print("\t 23. Activity surrounding sleep events")
        print("\t 24. XY coordinates surrounding sleep events")
        ...
        try:
            ...
            if option == 0:
                ...
                actPeriSleep(sleep, activity, shortFileName)

            elif option == 24:
                sleepKymo(sleep, X, Y, readsPerSec, shortFileName)
...
def openData():
    global pixPerCM
    file = getInput()
    activity, angularVelocity, readsPerSec, workList, shortFileName, X, Y, zeitStart, stim, feedData, pixPerCM, exptLengthHr = parseData(
        file)
    inactivity = getInactDuration(activity)
    sleep = getSleep(activity, readsPerSec, shortFileName)
    mealTimes, mealValues, mealDurations, boutCountList, mealEndTimes = autoCafe(feedData, readsPerSec, pixPerCM,
                                                                                 shortFileName)
...
def changeConstants():
    ...
    while True:
        ...
        if c == 1:
            ...
            sleepDef = int(input("Enter new sleep time minimum (s): "))
        elif c == 0:
            break
        else:
            print("input error.")
...
def parseData(file):
    ...
    for vector in range(len(foodTrack)):
        ...
        for read in range(0, len(foodTrack[vector]), feedFrameS):
            ...
            try:
                modeT = median(tempList)  # get median of population to clear any far outliers
        ...
        # Contributed by Scarlet Jinhong Park
        if scarletFilter:
            intervalList = []
            for i in range(4):
    ...
    for fly in range(len(X)):
        ...
        for read in range(len(X[fly])):
            ...
            try:
                ...
                singleDistance = 0
            if singleDistance < (bodyLenThresh * flyLength) / readsPerSec:
                if read > 0:
                    singleDistance = 0
                    X[fly][read] = X[fly][read - 1]
                    Y[fly][read] = Y[fly][read - 1]
        ...
        for f in range(flynum):
            for i in range(0, len(mealList)):  # This code generates two lists of both the meals and the zeit times
                try:
                    ...
                    indivMealEndTimes.append(
                        float(mealList[i][f + skip]) + float(float((mealList[i][f + skip + 2])) / 3600))
                except:
                    pass
...
def getSleep(activity, readsPerSec, shortFileName):
    ...
    global sleepDef
    sleepLength = int(readsPerSec * sleepDef)
    sleepGroup = []
    print("Sleep bout defined as %d seconds" % sleepDef)
    ...
    sleep = sleepGroup
    #toExcelNamed(sleep, "sleep_" + shortFileName)
    print("")
    print("")
...
def getSleepTimesZT(sleep, zeitStart):
    sleepZT = []
    for fly in range(len(sleep)):
        ...
        indivSleepIndex = []
        if len(sleep[fly]) > 0:
            while frame < len(sleep[fly]):
                try:
                    for second in range(len(sleep[fly])):
                        if sleep[fly][second] == 0 and sleep[fly][second + 1] == 1:
                            indivSleepZT.append(float(frame) / 3600 + zeitStart)
                            indivSleepIndex.append(second)
                        else:
                             frame += 1
                except:
                    break
            else:
                temp = [indivSleepZT, indivSleepIndex]
                sleepZT.append(temp)
        else:
            pass
    return sleepZT
...
def sleepKymo(sleep, X, Y, readsPerSec, shortFileName):
    # XY coordinates surrounding sleeping events

    zeitStart = getZeitStart()
    timeIndex = framesToSecs(readsPerSec, zeitStart, X)
    sleepTimesZT = getSleepTimesZT(sleep, zeitStart)
    timePush = int(input("Enter absolute time(sec) surrounding sleep events: "))
    step = input("Enter step interval (sec) surrounding sleep events: ")
    ZT = 0
    SI = 1
    try:
        step = int(step)
    ...
    x_List = []
    y_List = []
    for fly in range(len(sleepTimesZT)):
        for sleep in range(len(sleepTimesZT[fly][ZT])):
            try:
                current = sleepTimesZT[fly][SI][sleep]
                for n in range(-timePush, timePush, step):
                    try:
                        x_List.append(avg(X[fly][current + n:current + n + step]))
                        y_List.append(avg(Y[fly][current + n:current + n + step]))
                    except:
                        x_List.append("")
                        y_List.append("")

                x_List.insert(0, "X")
                x_List.insert(0, sleepTimesZT[fly][ZT][sleep])
                x_List.insert(0, sleep + 1)
                x_List.insert(0, fly + 1)
                finalList.append(x_List)
                x_List = []

                y_List.insert(0, "Y")
                y_List.insert(0, sleepTimesZT[fly][ZT][sleep])
                y_List.insert(0, sleep + 1)
                y_List.insert(0, fly + 1)
                finalList.append(y_List)
                y_List = []

            except:
                pass

    header = ["fly#", "sleep number", "sleep time", ""]
    for i in range(-timePush, timePush, step):
        header.append(str(i))
    finalList.insert(0, header)
    toExcelNamed(finalList,
                 "XY surrounding sleep_" + str(timePush) + "swindow_" + str(step) + "sbin_" + shortFileName)
...
def actPeriSleep(sleep, activity, shortFileName):
    zeitStart = getZeitStart()
    boutTimes = []
    boutLengths = []
    actPreSleep = []
    ...
    timePush = int(input("Enter absolute time (sec) surrounding sleep events: "))
    step = input("Enter step interval (sec) surrounding sleep events: ")
    lastBout = input("Do you want to consider the last sleep bout if cut off? (y/n)")
    ...
    for fly in range(len(sleep)):
        if sleep[fly][-1] == 1 and lastBout == 'y':
            sleep[fly].append(0)
        indivLength = []
        indivZT = []
        indivSleepPre = []
        ...
        if len(sleep[fly]) > 0:
            try:
                while read < len(sleep[fly]):
                    try:
                        if sleep[fly][read - 1] == 0 and sleep[fly][read] == 1:
                            ...
                            actPre = []
                            for n in range(-timePush, 0, step):
                                if read + n > 0:
                                    actPre.append(sum(activity[fly][read + n: read + n + step]))
                                else:
                                    actPre.append("")

                            indivSleepPre.append(actPre)
                            boutLen = 1
                            movingRead = read + 1
                            ...
                            for n in range(0, timePush, step):
                                if read + n < len(activity[fly]):
                                    actPost.append(sum(activity[fly][read + n:read + n + step]))
                                else:
                                    actPost.append("")
                            read = movingRead + 1
                            indivSleepPost.append(actPost)
                ...
                boutLengths.append(indivLength)
                boutTimes.append(indivZT)
                actPreSleep.append(indivSleepPre)
                actPostSleep.append(indivSleepPost)
    ...
    stepList = []
    for n in range(-timePush, timePush, step):
        stepList.append(str(n))

    final = [["fly#", "sleep time", "sleep length (s)"]]
    for n in range(-timePush, timePush, step):
        final[0].append(str(n))

    for fly in range(len(boutLengths)):
        try:
            for count in range(len(boutLengths[fly])):
                final.append([fly + 1, boutTimes[fly][count], boutLengths[fly][count]])
                for c in range(len(actPreSleep[fly][count])):
                    final[-1].append(actPreSleep[fly][count][c])
                for c in range(len(actPostSleep[fly][count])):
                    final[-1].append(actPostSleep[fly][count][c])
        except:
            pass

    toExcelNamed(final, "actPeriSleep_" + str(timePush) + "s_with_" + str(step) + "s_bins_" + shortFileName)
...
def groomGen(groomData, activity, inactivity, sleep, readsPerSec):
    ...
    # of grooming and motion for at least 5 minutes (unless sleepDef changed in change_constants())
    sleep = []
    sleepLength = int(readsPerSec * sleepDef)
    for fly in range(len(activity)):
        sleepIndiv = []
...
def autoCafe(feedData, readsPerSec, pixPerCM, shortFileName):
    try:
        ...
        preparedData, mealTimes, mealValues, mealDurations, mealEndTimes, boutCounts = prepareDataMatrix(mealData,
                                                                                                         ulConversion,
                                                                                                         frameOffset,
                                                                                                         intervalConversion)
        ...
        if roll == 'y' or roll == 'yes' or roll == 'Yes' or roll == 'YES':
            mealValues, mealDurations, mealTimes, boutCounts, mealEndTimes = rollBouts(mealValues, mealDurations,
                                                                                       mealTimes, boutCounts,
                                                                                       mealEndTimes)

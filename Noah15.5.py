	...
	while True:
		...
		try:
			...
			if option   == 0:
				...
				file = getInput()
				readsPerSec = 1/60
				mealTimes, mealValues, mealDurations, shortFileName = parseMealData(file)
...
def parseData(file):
	...
	return(activity,angularVelocity,readsPerSec,workList,shortFileName,X,Y,zeitStart,stim,feedData,pixPerCM,exptLengthHr)
...
def parseMealData(file):
	...
	if roll == 'y' or roll == 'yes' or roll == 'Yes' or roll == 'YES':
		...
		combineAutoCAFEMeals(mealTimes, mealValues, mealDurations, feedFileName)
...
def feedKymo(mealValues,mealDurations,mealTimes,X,Y,readsPerSec,shortFileName):
	...
	for fly in range(len(mealTimesZS)):
		...
		toExcelNamed(finalList,"XY surrounding meal_"+str(timePush)+"swindow_"+str(step)+"sbin_"+shortFileName)
...
def synth(sleep,activity,mealTimes,mealValues,mealDurations,readsPerSec,workList,useFileName,exptLengthHr):
	...
	# Many of these measurements will not show up in the output unless indicated in statsList at the end of this function
		...
		statsList = [flyCount,Activity,Sleep,Feed,FeedPerHr,numMeal,avgMeal,avgDuration,sleepBouts,avgSleepLength,sleepLatency,mealOnsetSleep,mealSleep,
...
def combineAutoCAFEMeals(mealTimes, mealValues, mealDurations, shortFileName):
	...
	for fly in range (len(mealTimes)):
		for meal in range (len(mealTimes[fly])):
			lines.append([fly+1,mealValues[fly][meal],mealDurations[fly][meal]*3600,mealTimes[fly][meal]])
	toExcelNamed(lines,"compressed_"+shortFileName)
